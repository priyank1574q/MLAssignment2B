...
for i in range(x_train.shape[0]):
    ...
    for j in range(len(x_train[i][0])):
        ...
        #word1 = stemma.stem(x_train[i][0][j])
        #word2 = stemma.stem(x_train[i][0][(j+1)%len(x_train[i][0])])
        #word1 = lemma.lemmatize(word1)
        #word2 = lemma.lemmatize(word2)
        #word1 = x_train[i][0][j]
        #word2 = x_train[i][0][(j+1)%len(x_train[i][0])]
        #x_train[i][0][j] = str(word1) + ' ' + str(word2)
        #vocab.update({str(word1) + ' ' + str(word2): 0})
...
for i in range(x_test.shape[0]):
    ...
    #for j in range(len(x_test[i][0])):
        #word1 = stemma.stem(x_test[i][0][j])
        #word2 = stemma.stem(x_test[i][0][(j+1)%len(x_test[i][0])])
        #word1 = lemma.lemmatize(word1)
        #word2 = lemma.lemmatize(word2)
        #word1 = x_test[i][0][j]
        #word2 = x_test[i][0][(j+1)%len(x_test[i][0])]
        #x_test[i][0][j] = str(word1) + ' ' + str(word2)
        #vocab.update({str(word1) + ' ' + str(word2): 0})
...
vocab = list(set(vocab).difference(stop))
